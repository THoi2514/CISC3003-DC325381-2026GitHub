...
REM   setup_database.bat full root 3306     -> full setup via root on custom port
REM =========================================================
...
if defined DB_PASS (
  "%MYSQL_EXE%" --protocol=tcp -h "%DB_HOST%" -P %DB_PORT% -u "%DB_USER%" -p"%DB_PASS%" ^< "%SQL_FILE%"
) else (
  "%MYSQL_EXE%" --protocol=tcp -h "%DB_HOST%" -P %DB_PORT% -u "%DB_USER%" ^< "%SQL_FILE%"
)
...
:ensure_database
echo [RUN] Ensuring database exists: %DB_NAME%
if defined DB_PASS (
  "%MYSQL_EXE%" --protocol=tcp -h "%DB_HOST%" -P %DB_PORT% -u "%DB_USER%" -p"%DB_PASS%" -e "CREATE DATABASE IF NOT EXISTS \`%DB_NAME%\` DEFAULT CHARACTER SET utf8mb4 DEFAULT COLLATE utf8mb4_unicode_ci;"
) else (
  "%MYSQL_EXE%" --protocol=tcp -h "%DB_HOST%" -P %DB_PORT% -u "%DB_USER%" -e "CREATE DATABASE IF NOT EXISTS \`%DB_NAME%\` DEFAULT CHARACTER SET utf8mb4 DEFAULT COLLATE utf8mb4_unicode_ci;"
)
if errorlevel 1 (
  echo [ERROR] Failed to create/check database "%DB_NAME%".
  echo         Verify host/port/user and privileges, then retry.
  exit /b 1
)
echo [OK] Database is ready: %DB_NAME%
echo.
exit /b 0
